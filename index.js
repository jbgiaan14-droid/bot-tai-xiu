...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('baucua')
        .setDescription('Chơi game Bầu Cua Tôm Cá đổi vận!')
        .addStringOption(option =>
            option.setName('luachon')
                .setDescription('Chọn con vật bạn muốn đặt cược')
                .setRequired(true)
                .addChoices(
                    { name: 'Bầu', value: 'Bầu' },
                    { name: 'Cua', value: 'Cua' },
    ...
    async execute(interaction) {
        const luaChon = interaction.options.getString('luachon');
        const tienCuoc = interaction.options.getInteger('tiencuoc');

        // Lắc 3 con xúc xắc ngẫu nhiên
        const ketQua = [
        ...
        // Đếm số lần xuất hiện của con vật người chơi chọn
        const soLanTrung = ketQua.filter(item => item === luaChon).length;

        let embed = new EmbedBuilder()
            .setTitle('🎲 KẾT QUẢ BẦU CUA TÔM CÁ 🎲')
            .setColor(soLanTrung > 0 ? 0x00FF00 : 0xFF0000)
            .addFields(
                { name: 'Người chơi chọn', value: `${EMOJI_MAP[luaChon]} **${luaChon}** (Cược: ${tienCuoc} VNĐ)`, inline: false },
                { name: 'Kết quả xúc xắc', value: `1. ${EMOJI_MAP[ketQua[0]]} **${ketQua[0]}**\n2. ${EMOJI_MAP[ketQua[1]]} **${ketQua[1]}**\n3. ${EMOJI_MAP[ketQua[2]]} **${ketQua[2]}**`, inline: false }
            );

        if (soLanTrung > 0) {
            const tienThang = tienCuoc * soLanTrung;
            embed.addFields({ name: 'Kết quả', value: `🎉 Chúc mừng! Ra **${soLanTrung}** nháy **${luaChon}**. Bạn húp trọn **+${tienThang} VNĐ**!`, inline: false });
        } else {
            embed.addFields({ name: 'Kết quả', value: `😢 Chia buồn, không trúng con nào cả. Bạn mất **-${tienCuoc} VNĐ**!`, inline: false });
        }
